		...
		methods: {
			...
			chooseImage: function(index) {
				...
				uni.chooseImage({
					...
					sourceType: ['album'],//sourceType: sourceType,
					success: function(res) {
						// TODO 真是上传照片
						console.log(res);
						_this.setData({
						...
						success: function (res) {
							...
							uni.compressImage({
							    ...
							    success: function (compressres) {
									...
									//转成base64存储
									_this.pathToBase64(compressres.tempFilePath).then(base64_res => {
										_this.$emit('uploadSuccess', base64_res)
									}).catch(err=>{
										console.log('转换64报错', err)
									});	
